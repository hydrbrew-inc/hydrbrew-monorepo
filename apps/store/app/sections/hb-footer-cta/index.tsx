...
function HbFooterCta(props: HydrogenComponentProps) {
  ...
  return (
    <section {...rest} className="py-20 md:py-32 bg-black" style={{ borderTop: "1px solid rgba(0,255,255,0.15)" }}>
      <div className="max-w-2xl mx-auto px-4 text-center mb-20">
        ...
        <h2 className="text-5xl md:text-7xl font-bold text-white mb-4" style={{ fontFamily: "'Urbanist',sans-serif" }}>
          Establish <span style={{ color: "#00FFFF" }}>Uptime Baseline</span>
        </h2>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          />
          <button className="px-8 py-4 rounded-full font-bold text-black uppercase tracking-wider transition-all hover:opacity-90" style={{ backgroundColor: "#00FFFF", fontFamily: "'Urbanist',sans-serif" }}>
            INITIALIZE
          </button>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4">
          ...
          {[
            ...
            <div key={col.heading}>
              ...
              <ul className="space-y-2">
                {col.links.map((link) => (
                  <li key={link}>
                    <span className="text-sm text-white/60 hover:text-[#00FFFF] cursor-pointer transition-colors" style={{ fontFamily: "'Urbanist',sans-serif" }}>{link}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-8" style={{ borderTop: "1px solid rgba(0,255,255,0.1)" }}>
          <div>
          ...
          <p className="text-xs text-white/40" style={{ fontFamily: "'Urbanist',sans-serif" }}>© 2026 hydrbrew°. All rights reserved.</p>
          <div className="flex gap-3">
            {[{ label: "IG", href: "https://www.instagram.com/tryhydrbrew/" }, { label: "X", href: "https://x.com/hydrbrew" }, { label: "YT", href: "https://www.youtube.com/@hydrbrew" }].map((s) => (
              <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full flex items-center justify-center text-xs text-white/60 hover:text-[#00FFFF] transition-colors" style={{ border: "1px solid rgba(0,255,255,0.2)" }}>
                {s.label}
              </a>
            ))}
          </div>
